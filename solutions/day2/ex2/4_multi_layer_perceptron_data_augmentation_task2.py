...
import functools
from cnn_batch_norm_improved import CNNBatchNormImproved
...
images, labels = utils.load_cifar(os.path.join(cifar_dir, 'test'))
...
def random_center_crop(image, target, dim=(20, 20), probability=.5):
    x, y, z = image.shape
    ...
    if np.random.rand() < probability:
        ...
        cropx = (x - dim[0]) // 2
        cropy = (y - dim[1]) // 2
        res[cropx:cropx+dim[0],cropy:cropy+dim[1], ...] = image[cropx:cropx+dim[0],cropy:cropy+dim[1], ...]
        image = res
    return image, target


def random_grayscale(image, target, probability = .5):
    x, y, z = image.shape
    if np.random.rand() < probability:
        res = np.apply_along_axis(lambda c: .299 * c[0] + .587 * c[1] + .114 * c[2], 0, image)
        res = np.stack((res,)*3, axis=0)
        res = np.cast["float32"](res)
        image = res
    return image, target
...
if not os.path.exists("checkpoints/best_checkpoint_SimpleCNN_augmented.tar" ):
    utils.run_cifar_training(model, optimizer,
            ...
            device=device, name='SimpleCNN_augmented', 
            n_epochs=n_epochs)
...
model = utils.load_checkpoint("best_checkpoint_SimpleCNN_augmented.tar", model, optimizer)[0]
...
model = CNNBatchNormImproved(10)
model = model.to(device)
optimizer =torch.optim.Adam(model.parameters(), lr=1.e-3)

if not os.path.exists("checkpoints/best_checkpoint_CNNBatchNormImproved_augmented.tar"):
    utils.run_cifar_training(model, optimizer,
            train_loader, val_loader,
            device=device, name='CNNBatchNormImproved_augmented', 
            n_epochs=n_epochs)

model = utils.load_checkpoint("best_checkpoint_CNNBatchNormImproved_augmented.tar", model, optimizer)[0]
