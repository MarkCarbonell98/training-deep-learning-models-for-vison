...
lrs = [1.e-2, 1.e-3, 1.e-4, 1.e-5]
data = []
for i, lr in enumerate(lrs):
    model = MLP(3072, 10)
    model.to(device)
    optimizer = torch.optim.Adam(model.parameters(), lr=lr)
    tb_logger = torch.utils.tensorboard.SummaryWriter('runs/log_mlp_{lr}'.format(lr=lr))
    n_epochs = 4
    ...
    print("Training time for MLP with lr = {lr}: {ttime}ms".format(lr=lr, ttime=stop))
    ...
    ax.set_title("MLP with learning rate {lr}".format(lr=lr))
...
plt.savefig(os.path.abspath(os.path.join('solutions', 'day1', 'ex3', 'ex3_confusion_matrices.png')) ,format='png')
...
plt.savefig(os.path.abspath(os.path.join('solutions', 'day1', 'ex3', 'ex_3_model_data.png')) ,format='png')
